...
//    }
    ...
    //Signal(SIGTERM, sigterm_handler);  /* Kill the process */
...
void eval(char *cmdline)
{
    ...
    if(retval==0){                      //if not builtin_cmd
        ...
        //parent
            ...
            sigprocmask(SIG_UNBLOCK, &set, NULL);   //unblock signal SIGCHILD
            if(bg)
                printf("[%d] (%d)\n", pid2jid(pid), pid);     //print background job info
            else
...
int builtin_cmd(char **argv)
{
    if(!strcmp(argv[0],"exit")){        //quit the job if there are no stopped jobs in job list
        struct job_t *job=jobs;
        int i=0;
        for(i=0;i<MAXJOBS;i++){
            if(job[i].state==ST){
//                printf("You can't exit, there are still stopped jobs in job-table\n");
                 kill((job[i].pid),SIGHUP);
                 kill((job[i].pid),SIGCONT);
//                 deletejob(jobs,job[i].pid);
            }
            else if(job[i].state==FG || job[i].state==BG){
                kill((job[i].pid),SIGHUP);
//                deletejob(jobs,job[i].pid);
            }
        }
